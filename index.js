...
function processContains(item, list, callback) {
  const match = (element) => element === item;
  return callback(list.some(match))
}; 
...
function processDuplicateFree(list, callback) {
  ...
  console.log(array);
  return callback(Array.from(array));
};


// const newArray = list.filter((item, index) => list.indexOf(item) === index);
...
function tallyUpDonations(runners) {
  const totalDonation = runners.reduce(function(accumulator, item){
    return accumulator + item.donation;
  }, 0);return totalDonation;
}
